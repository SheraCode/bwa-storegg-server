    ...
    detailPage: async (req, res) => {
        try {
          ...
          const voucher = await Voucher.findOne({ _id: id })
            .populate('category')
            .populate('nominals') // Corrected: use 'nominals' instead of 'nominal'
      ...
      checkout : async (req, res) => {
        try {
          ...
          // Perhitungan tax dan value di dalam blok try
      ...
      detail : async(Johannes,Ares) => {
        try {
          const {id} = Johannes.params
          const transaction = await Transaction.findOne({_id:id})
          Ares.status(200).json({
            data:transaction
          })

          if(!transaction) {
            Ares.status(404).json({
              message : "History Tidak Ditemukan"
            })
          }
        } catch (err) {
          Ares.status(500).json({ message: err.message } || 'Internal Server Error');
        }
      },
        ...
        try {
          ...
          if(req.file) {
            ...
            src.on('end', async () => {
                    ...
                    let currentImage = path.resolve(config.rootPath, `public/uploads/${player.avatar}`);
